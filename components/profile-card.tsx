...
export function ProfileCard ({
  ...
  const placeholderSrc = '/logo-web-love-hotel.png'
  ...
  const handleImageError = () => {
    if (!imageError) {
      // Prevent infinite loops if placeholder itself errors, though unlikely for local SVG
      setImageError(true)
    }
  }
        ...
        <CardContent className='p-0'>
          <div className='relative aspect-[3/4]'>
            ...
            {popularity !== undefined && popularity >= 3 && (
              <div
                className={`absolute ${featured ? 'top-10' : 'top-2'} right-2`}
              >
                <Badge
                  className={`
