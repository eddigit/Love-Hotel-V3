...
import { getUpcomingEvents, subscribeToEvent, unsubscribeFromEvent } from "@/actions/event-actions"
...
export default function EventsPage(props) {
  ...
  useEffect(() => {
    ...
    async function fetchEventsAndCategories() {
      ...
      setEvents(result)
      const lines = (rawCategories || "speed-dating|Speed Dating\njacuzzi|Jacuzzi\nrestaurant|Restaurant").split("\n")
      setCategories(
        lines
          .map(line => line.trim())
          .filter(Boolean)
  ...
  const handleSubscribeToggle = async (event: any) => {
    ...
    if (event.is_participating) {
      ...
      setEvents(events.map(e => e.id === event.id ? { ...e, is_participating: false, attendees: e.attendees - 1 } : e))
    } else {
      await subscribeToEvent(event.id, authUser.id)
      setEvents(events.map(e => e.id === event.id ? { ...e, is_participating: true, attendees: e.attendees + 1 } : e))
    }
  }
      ...
      <div className="min-h-screen flex flex-col pb-16 md:pb-0">
        <div className="container py-4 md:py-6 flex-1">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className={`grid w-full grid-cols-${categories.length + 1} mb-4 md:mb-6`}>
            ...
            {categories.map(cat => (
              <TabsContent key={cat.value} value={cat.value} className="space-y-4 md:space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {loading ? (
                    <div>Chargement...</div>
                  ) : (
