import { getUserProfile, getMatchStatus, sendMatchRequest, acceptMatchRequest, declineMatchRequest } from "@/actions/user-actions"
    ...
    <MainLayout session={session} user={currentUser}>
      <div className="min-h-screen flex flex-col pb-16 md:pb-0">
        ...
        <div className="container mt-24 md:mt-28 py-6 flex-1">
          <Card className="mb-6 border-0 bg-gradient-to-br from-[#2d1155]/70 to-[#3d1155]/50 backdrop-blur-sm shadow-lg shadow-purple-900/20">
            <CardContent className="p-6">
              <div className="space-y-4">
                ...
                {formattedBirthday && (
                  <div>
                    <h3 className="font-semibold mb-2">Date de naissance</h3>
                    <div className="text-muted-foreground">{formattedBirthday}</div>
                  </div>
                )}
                {profile.gender && (
                  <div>
                    <h3 className="font-semibold mb-2">Genre</h3>
                    <div className="text-muted-foreground">{profile.gender}</div>
                  </div>
                )}
                {profile.orientation && (
                  <div>
                    <h3 className="font-semibold mb-2">Orientation</h3>
                    <div className="text-muted-foreground">{profile.orientation}</div>
                  </div>
                )}
              </div>
              {/* Match logic UI */}
              {currentUser && currentUser.id !== user.user_id && (
                <div className="mt-6">
                  {matchStatus === null && (
                    <MatchRequestButton currentUserId={String(currentUser.id)} profileUserId={String(user.user_id)} />
                  )}
                  {matchStatus && matchStatus.status === "pending" && isRequester && (
                    <Button disabled className="bg-gray-400">Demande envoyée (en attente)</Button>
                  )}
                  {matchStatus && matchStatus.status === "pending" && !isRequester && (
                    <div className="flex gap-2">
                      <form action={async () => {
                        'use server';
                        const accepted = await acceptMatchRequest(matchStatus.user_id_1, matchStatus.user_id_2);
                        if (accepted?.success) {
                          const conversationId = await findOrCreateConversation(matchStatus.user_id_2, matchStatus.user_id_1);
                          if (conversationId) {
                            redirect(`/messages/${conversationId}`);
                          }
                        }
                      }}>
                        <Button type="submit" className="bg-green-500">Accepter</Button>
                      </form>
                      <form action={async () => { 'use server'; await declineMatchRequest(matchStatus.user_id_1, matchStatus.user_id_2) }}>
                        <Button type="submit" className="bg-red-500">Refuser</Button>
                      </form>
                    </div>
                  )}
          ...
          {/* You can add more tabs/sections here for events, photos, etc. */}
        </div>
        <MobileNavigation />
      </div>
    </MainLayout>
